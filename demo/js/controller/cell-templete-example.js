...
.controller('instanceApp.controller', function($scope, instancesData, $timeout) {
	
	//给表格中按钮定义的函数
	//请放在表格定义模块之上
	$scope.selectWorklist = function() {
		//由于触发的顺序，导致本函数在选中列表行函数之前执行，故而延时100毫秒再获取选中行对象
		$timeout(function() {
			alert("请注意看控制台");
			console.log( "选中的行对象信息" +$scope.mySelections);
	...
	$scope.angridOptions = {
		...
		[{
			...
			displayName : '虚拟机名称',
			cssClass : 'col1',
			columnTemplete : '<input type="text" ng-model="rowData[colData.field]" class="span1" />'
		}, {
			field : 'private_ip',
			displayName : '内网IP',
			cssClass : 'col2'
			...
			displayName : '快照',
			cssClass : 'col11',
			columnFuc : $scope.selectWorklist, //将行内按钮的函数传入angrid，
			columnTemplete : '<button class="btn btn-mini" type="button" ng-click="colData.columnFuc()">编辑与详情</button>'//在表格内，触发额度函数名字叫colData.columnFuc()
