...
angular.module('instanceApp', ['instanceApp.services', 'commonFilter', 'anGrid'])
	.controller('instanceApp.controller', function ($scope, instancesData) {
		...
		$scope.coldefs = [
			{ field: 'private_ip', displayName:'内网IP', enableCellEditOnFocus: true}
			,{ field: 'flavor_name', displayName:'配置信息', enableCellEditOnFocus: true}
			,{ field: 'billing_type', displayName:'计费类型', enableCellEditOnFocus: true}
			,{ field: 'status', displayName:'状态', columnFilter: 'instance_status', enableCellEditOnFocus: true}
			,{ field: 'ssh_desp', displayName:'公网SSH域名', enableCellEditOnFocus: true}
			,{ field: 'floating_ips', displayName:'公网ip', enableCellEditOnFocus: true}
			,{ field: 'key', displayName:'秘钥', enableCellEditOnFocus: true}
			,{ field: 'security_group', displayName:'安全组', enableCellEditOnFocus: true}
			,{ field: 'image', displayName:'镜像', enableCellEditOnFocus: true}
			,{ field: 'snap', displayName:'快照', enableCellEditOnFocus: true}
		];
		$scope.angridOptions = {
			data:                        'myData', //数据输入
